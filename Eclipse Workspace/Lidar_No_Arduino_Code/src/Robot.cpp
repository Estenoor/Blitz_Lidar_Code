...
class Robot: public SampleRobot
{
	...
	FRC::DashboardManager Dashboard;
	//Relay spike;
	Timer *lightTimer;
		...
		//spike(0)
{
	...
	void Autonomous()
	{
		...
		visionThread.detach();
		bool gotAngle = false;
		double Angle = driveMan.getAngle();
		driveMan.resetEnc();
		...
		bool stepFour = false;
		double autoJoystickVal = 0.3;

		while (IsAutonomous() && IsEnabled())
		{
			//Autonomous SmartDashboard
			Dashboard.autoSmartDashboard();

			Wait(.005);
			time += .005;

			//Autonomous Protocol 1
			if(autoMan.Auto.GetRawButton(1) && !autoMan.Auto.GetRawButton(2) && !autoMan.Auto.GetRawButton(3))
			{
				if(stepOne && time < 5)
				{
					driveMan.straightDrive(Angle, 0, autoJoystickVal);
				}
				else if(stepOne)
				{
					stepTwo = true;
				...
				if(stepTwo && driveMan.getAngle() < 115)
				{
					driveMan.mecanumDrive(0, 0, autoJoystickVal, false);
				}
				else if(stepTwo)
				{
					stepThree = true;
			...
			else if(!autoMan.Auto.GetRawButton(1) && !autoMan.Auto.GetRawButton(2) && autoMan.Auto.GetRawButton(3))
			{
				...
				if(stepTwo && driveMan.getAngle() < 235)
				{
					driveMan.mecanumDrive(0, 0, autoJoystickVal, false);
				}
				else if(stepTwo)
	...
	void OperatorControl()
	{
		...
		while (IsOperatorControl() && IsEnabled())
		{
			...
			{ //if(manipulator.PDP.GetCurrent(0) > 50 && manipulator.PDP.GetCurrent(1) < 50){
...
/*
The below measurements are approximate, use only for starting the frame of the autonomous code.
